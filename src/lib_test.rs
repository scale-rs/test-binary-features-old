...
fn has_error() {
    ...
    assert!(!crate::has_error(&Some(ok), &None));

    let failed = output_failed();
    let failed_status = failed.status;
    assert!(crate::has_error(&Some(failed), &None));
    ...
    assert!(crate::has_error(&ok_outputs_but_failed_status, &None));
    ...
    assert!(crate::has_error(&failed_outputs_but_ok_status, &None));

    assert!(crate::has_error(&None, &Some(Box::new(Err {}))));
}
